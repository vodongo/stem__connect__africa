...
function Forgotpassword(props, reset, error) {
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
            <Formik
                ...
                onSubmit={async (values, { setSubmitting }) => {
                    ...
                    await props.handle__forgot__password(values)
                    if (reset === true) {
                        setValid(false)
                    } else {
                        setValid(false)
            ...
            >
                <Form className="grid grid-cols-1 max-w-sm w-full space-y-2 bg-white px-8 shadow rounded py-6">
                    <div className="space-y-2">
                        <div className="text-2xl text-gray-800 font-medium text-center">Forgot Your Password</div>
                        <p>Enter your <span className="border-b-2 border-gray-600 border-dashed">primary email address</span> and we’ll send you instructions on how to reset your password.</p>
                    </div>

                    <div>
                        {error && <div class="bg-red-100 rounded-lg">
                            <div className="max-w-sm mx-auto py-2 px-3 sm:px-6 lg:px-6">
